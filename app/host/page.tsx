...
export default function HostPage() {
	...
	const start = () => {
		sfx.play("click");
		code && getSocket().emit("host:start", { code, hostKey });
	};

	const endRound = () => {
		sfx.play("click");
		code && getSocket().emit("host:end_round", { code, hostKey });
	};

	const callNext = () => {
		sfx.play("click");
		code && getSocket().emit("host:call_next", { code, hostKey });
	};

	const undo = () => {
		sfx.play("click");
		code && getSocket().emit("host:undo", { code, hostKey });
	};
					...
					<div className='grid gap-6 lg:grid-cols-[1fr,1fr]'>
						<div className='card p-5 space-y-4'>
							...
							<PatternPicker
								value={summary.pattern}
								onChange={p =>
									getSocket().emit("host:set_pattern", {
										code,
										hostKey,
										pattern: p
									})
							...
							<div className='grid gap-3 sm:grid-cols-3'>
								<Toggle
									checked={summary.allowAutoMark}
									onChange={v =>
										getSocket().emit("host:set_allow_automark", {
											code,
											hostKey,
											allow: v
										})
								...
								<Toggle
									checked={summary.lockLobbyOnStart}
									onChange={v =>
										getSocket().emit("host:set_lock_on_start", {
											code,
											hostKey,
											lockOnStart: v
										})
								...
								<Toggle
									checked={summary.locked}
									onChange={v =>
										getSocket().emit("host:set_locked", {
											code,
											hostKey,
											locked: v
										})
