...
export default function HostPage() {
	...
	useEffect(() => {
		...
		socket.on("game:winner", (w: any) => {
			setWinners(prev => {
				...
				return [
					...prev,
					{ playerId: w.playerId, name: w.name, pattern: w.pattern, round, cardIndex: w.cardIndex }
				];
			});
	...
	const currentRoundWinners = useMemo(() => winners.filter(w => w.round === round), [winners, round]);
						...
						<div className='flex items-center gap-2 flex-wrap'>
							...
							{currentRoundWinners.length === 0 ? (
								...
								currentRoundWinners.map(w => (
									<span
										key={w.playerId}
										className='px-3 py-1 rounded-2xl text-white bg-gradient-to-r from-emerald-500 to-emerald-700'>
										{w.name}{" "}
